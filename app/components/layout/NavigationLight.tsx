...
import PerfumePopup from '../sections/PerfumeFinder2'; 

const megaMenuData = {
  extraitDeParfum: {
    label: 'Extrait de Perfume',
    columns: [
      {
        heading: 'Man',
        items: [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Bravehert',  image: '/mega-menu/09.jpg', href: '/braveheart-womens-perfume-55ml' },
...
export default function NavigationLight() {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-xl ${BG} border-b border-black/8 transition-all duration-500`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between lg:grid lg:grid-cols-3">
            ...
            <div className="flex justify-start">
              <Link href="/" className="relative block w-24 sm:w-28 md:w-32 h-7 sm:h-8 md:h-10">
                <Image src="/logo-black.png" alt="Logo" fill className="object-contain object-left" priority />
              </Link>
            </div>

            <div className="hidden lg:flex justify-center items-center gap-5 xl:gap-7">
              <Link href="/about" className="relative text-black text-[13px] xl:text-[14px] tracking-[0.15em] font-glacial group whitespace-nowrap">
                About
                <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-black transition-all duration-300 group-hover:w-full" />
              </Link>

              <div className="relative" onMouseEnter={openMega} onMouseLeave={closeMega}>
                <button className={`relative text-black text-[13px] xl:text-[14px] tracking-[0.15em] font-glacial flex items-center gap-1 whitespace-nowrap ${megaOpen ? 'opacity-100' : ''}`}>
                  Fragrance
                  <span className={`absolute -bottom-1 left-0 h-[1px] bg-black transition-all duration-300 ${megaOpen ? 'w-full' : 'w-0'}`} />
                </button>
              </div>

              <button onClick={() => setIsPerfumeFinderOpen(true)} className="relative text-black text-[13px] xl:text-[14px] tracking-[0.15em] font-glacial group whitespace-nowrap">
                Discover Your Signature Fragrance
                <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-black transition-all duration-300 group-hover:w-full" />
              </button>

              <Link href="/contact" className="relative text-black text-[13px] xl:text-[14px] tracking-[0.15em] font-glacial group whitespace-nowrap">
                Contact Us
                <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-black transition-all duration-300 group-hover:w-full" />
              </Link>
            </div>

            <div className="flex justify-end items-center gap-3 sm:gap-4 text-black">
              <Link href="/login" aria-label="Login" className="hover:opacity-40 transition-opacity">
                <Icon name="person_outline" className="text-[20px] sm:text-[22px] md:text-[26px]" />
              </Link>
              <Link href="/cart" aria-label="Cart" className="hover:opacity-40 transition-opacity relative">
                <Icon name="shopping_cart" className="text-[20px] sm:text-[22px] md:text-[26px]" />
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold text-white font-glacial bg-black">
                    {cartCount}
                  </span>
                )}
              </Link>
              <button onClick={() => setIsMenuOpen(true)} aria-label="Open Sidebar" className="hidden lg:block hover:opacity-40 transition-all active:scale-90">
                <Icon name="grid_view" className="text-[24px] md:text-[28px]" />
              </button>
              <button onClick={() => setMobileNavOpen(true)} aria-label="Open Mobile Menu" className="lg:hidden hover:opacity-40 transition-all active:scale-90">
        ...
        {/* MEGA MENU — same wrapper, no separate bg/blur needed */}
        <div
              ...
              <div className="flex-1 flex gap-12 xl:gap-16">
                <div className="flex-1">
                  <div className="flex items-center gap-5 mb-10">
                    ...
                    <span className="text-[12px] xl:text-[14px] tracking-[0.4em] uppercase text-black/80 font-glacial whitespace-nowrap">Extrait de Fragrance</span>
                    <div className="flex-1 h-[1px] bg-black/20" />
                  </div>
                  <div className="grid grid-cols-3 gap-6 xl:gap-8">
                    {megaMenuData.extraitDeParfum.columns.map((col) => (
                      <div key={col.heading}>
                        <p className="text-[10px] xl:text-[11px] tracking-[0.3em] uppercase text-black/30 font-glacial mb-3 pb-2 border-b border-black/10 font-bold">{col.heading}</p>
                        <ul className="space-y-2.5 xl:space-y-3">
                          {col.items.map((item) => (
                            <li key={item.name}>
                              <Link
                                href={item.href}
                                className="block text-black/65 tracking-[0.1em] text-[14px] xl:text-[15px] font-glacial transition-all duration-200 hover:text-black hover:translate-x-1.5"
                ...
                <div className="w-[200px] xl:w-[220px] flex-shrink-0">
                  <div className="flex items-center gap-3 mb-10">
                    <div className="flex-1 h-[1px] bg-black/20" />
                    <span className="text-[12px] xl:text-[14px] tracking-[0.4em] uppercase text-black/80 font-glacial whitespace-nowrap">Aroma Pod</span>
                    <div className="flex-1 h-[1px] bg-black/20" />
                  </div>
      ...
      <div className={`fixed inset-0 z-[150] lg:hidden transition-all duration-500 ${mobileNavOpen ? 'visible' : 'invisible'}`}>
        <div className={`absolute inset-0 bg-black/30 backdrop-blur-sm transition-opacity duration-500 ${mobileNavOpen ? 'opacity-100' : 'opacity-0'}`} onClick={() => setMobileNavOpen(false)} />
        <div className={`absolute top-0 left-0 h-full w-[80vw] max-w-xs ${BG} backdrop-blur-xl shadow-2xl flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between px-6 py-5 border-b border-black/10">
            <div className="relative w-20 h-6">
              <Link href="/" className="relative block w-24 sm:w-28 md:w-32 h-7 sm:h-8 md:h-10">
                <Image src="/logo-black.png" alt="Logo" fill className="object-contain object-left" />
              </Link>
            </div>
            <button onClick={() => setMobileNavOpen(false)} className="text-black/40 hover:text-black hover:rotate-90 transition-all duration-300 p-1">
              <Icon name="close" className="text-2xl" />
            </button>
          </div>
          <nav className="flex-1 min-h-0 overflow-y-auto px-6 py-6 space-y-1">
            <Link href="/about" className="block text-black text-[16px] font-glacial py-3 border-b border-black/5 tracking-widest">About</Link>
            <div className="border-b border-black/5">
              <button onClick={() => setMobilePerfumOpen(!mobilePerfumOpen)} className="w-full flex items-center justify-between text-black text-[16px] font-glacial py-3 tracking-widest">
                Fragrance
              ...
              <div className={`overflow-y-auto transition-all duration-300 ${mobilePerfumOpen ? 'max-h-[60vh] pb-4' : 'max-h-0'}`}>
                <div className="mt-3 mb-1">
                  <p className="text-[10px] tracking-[0.4em] uppercase text-black/30 font-glacial mb-3 px-2">Extrait de Parfum</p>
                  {megaMenuData.extraitDeParfum.columns.map((col) => (
                    <div key={col.heading} className="mt-3">
                      <p className="text-[11px] tracking-[0.3em] uppercase text-black/40 font-glacial mb-2 px-2 font-bold">{col.heading}</p>
                      <ul className="space-y-1">
                        {col.items.map((item) => (
                          <li key={item.name}>
                            <Link href={item.href} className="block text-black/65 text-[15px] font-glacial py-1.5 px-2 rounded hover:text-black transition-all" onClick={() => setMobileNavOpen(false)}>
                              {item.name}
                            </Link>
                          </li>
                        ))}
                ...
                <div className="my-4 mx-2 h-[1px] bg-black/10" />
                <div>
                  <p className="text-[10px] tracking-[0.4em] uppercase text-black/30 font-glacial mb-3 px-2">Aroma Pod</p>
                  {megaMenuData.aromaPod.columns.map((col) => (
                    <div key={col.heading} className="mt-3">
                      <p className="text-[11px] tracking-[0.3em] uppercase text-black/40 font-glacial mb-2 px-2 font-bold">{col.heading}</p>
                      <ul className="space-y-1">
            ...
            <button onClick={() => { setMobileNavOpen(false); setIsPerfumeFinderOpen(true); }} className="w-full text-left block text-black text-[16px] font-glacial py-3 border-b border-black/5 tracking-widest">Discover Your Signature Fragrance</button>
            <Link href="/contact" className="block text-black text-[16px] font-glacial py-3 border-b border-black/5 tracking-widest">Contact Us</Link>
          </nav>
          <div className="px-6 py-5 border-t border-black/10">
            <p className="text-[10px] tracking-[0.3em] uppercase text-black/30 font-glacial mb-3">Follow Us</p>
            <div className="flex items-center gap-5">
              {[Facebook, Instagram, MessageCircle, Twitter, Linkedin].map((Icon2, i) => (
                <a key={i} href="#" className="text-black/40 hover:text-black transition-colors"><Icon2 className="w-4 h-4" /></a>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className={`fixed inset-0 z-[100] transition-all duration-500 ${isMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-full max-w-sm bg-zinc-950 shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] p-10 flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
